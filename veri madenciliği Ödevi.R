...
cutoff = 0.5
pred = predict(fit.pruned, newdata=heart, type="prob") 
yhat = ifelse(pred[,2] > cutoff, 1, 0)
ctable = table(heart$target, yhat, dnn=c("Actual", "Predicted")); ctable 

 
miss.err = 1-sum(diag(ctable))/sum(ctable); miss.err
pred.acc = 1 - miss.err; pred.acc 
diag(ctable)[2]/apply(ctable, 1, sum)[2] 
diag(ctable)[1]/apply(ctable, 1, sum)[1] 

library(ROCR) 

pred2 = predict(fit.pruned, newdata=heart, type="prob") 
pred = prediction(pred2[,2], heart$target)
perf = performance(pred, "tpr","fpr")

plot(perf, col = 4, lwd = 2, xlab = "1-Specificity", ylab = "Sensitivity", main = "ROC Curve")
...
cutoff = 0.5
pred = predict(fit.pruned, newdata=heart.test, type="prob")
yhat = ifelse(pred[,2] > cutoff, 1, 0)
ctable = table(heart.test$target, yhat, dnn=c("Actual", "Predicted")); ctable 
...
pred2 = predict(fit.pruned, newdata=heart.train, type="prob") 
...
pred2 = predict(fit.pruned, newdata=heart.test, type="prob") 
pred = prediction(pred2[,2], heart.test$target)
perf = performance(pred, "tpr","fpr")

plot(perf, col = 4, lwd = 2, xlab = "1-Specificity", ylab = "Sensitivity", main = "ROC Curve (Test)") 
lines(x = c(0,1), y = c(0,1), col = 2, lty = 2, lwd = 2)
legend(cex=0.5, "bottomright", legend = c("Tree","Random"), col = c(4,2), lty = c(1,2), lwd = 2)
